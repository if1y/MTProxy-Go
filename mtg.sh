...
Uninstall(){
	...
	if [[ ${unyn} == [Yy] ]]; then
		...
		if [[ ! -z $(crontab -l | grep "mtg.sh monitor") ]]; then
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://github.com/if1y/MTProxy-GO_OnekeyMake/raw/dev/mtg.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/mtproxy-go" ]]; then
		rm -rf /etc/init.d/mtproxy-go
		Service
	fi
	wget -N --no-check-certificate "https://github.com/if1y/MTProxy-GO_OnekeyMake/raw/dev/mtg.sh" && chmod +x mtg.sh
